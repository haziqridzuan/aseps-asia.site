  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Total Projects" value={projects.length} className="bg-blue-50" />
        <StatCard title="Clients" value={clients.length} className="bg-green-50" />
        <StatCard title="Suppliers" value={suppliers.length} className="bg-amber-50" />
        <StatCard title="Purchase Orders" value={purchaseOrders.length} className="bg-purple-50" />
      </div>
          ...
          <CardContent>
            <dl className="space-y-4">
              ...
              <div className="flex justify-between">
                ...
                <dd className="font-bold">
                  {purchaseOrders.filter((po) => po.status === 'Delayed').length}
                </dd>
              </div>
              <div className="flex justify-between">
          ...
          <CardContent>
            <dl className="space-y-4">
              ...
              <div className="flex justify-between">
                ...
                <dd className="font-bold">
                  {externalLinks.filter((link) => link.type === 'Report').length}
                </dd>
              </div>
              <div className="flex justify-between">
                <dt className="font-medium text-muted-foreground">Photos</dt>
                <dd className="font-bold">
                  {externalLinks.filter((link) => link.type === 'Photo').length}
                </dd>
              </div>
              <div className="flex justify-between">
                <dt className="font-medium text-muted-foreground">Tracking</dt>
                <dd className="font-bold">
                  {externalLinks.filter((link) => link.type === 'Tracking').length}
                </dd>
              </div>
            </dl>
          </CardContent>
      ...
      <Card className="card-hover">
        ...
        <CardContent className="space-y-4">
          <p className="text-muted-foreground">
            Use the Admin Panel to manage all data in the ASEPS Asia system. You can add, edit, and
            delete projects, clients, suppliers, purchase orders, and external links.
          </p>
