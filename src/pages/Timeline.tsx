...
export default function Timeline() {
  ...
  const projectPOs = selectedProject
    ? purchaseOrders.filter((po) => po.projectId === selectedProject.id)
    : [];
  ...
  const generatePOTimeline = (po: PurchaseOrder): TimelineEvent[] => {
    ...
    po.parts.forEach((part, index) => {
      ...
      partDate.setDate(
        startDate.getDate() + Math.floor(totalDays * ((index + 1) / (po.parts.length + 1))),
      );
      ...
      <Card className="card-hover">
        <CardHeader className="pb-2">
          <CardTitle className="text-lg font-medium">
            {selectedPO
              ? `Timeline for PO ${selectedPO.poNumber}`
              : selectedProject
                ? `Timeline for Project ${selectedProject.name}`
            ...
            <div className="relative">
              ...
              <div className="space-y-8">
                {timeline.map((event, index) => (
                  ...
                  >
                    <div
                      className={`w-6 h-6 rounded-full flex items-center justify-center z-10 ${getIconColor(event.type)}`}
                    >
