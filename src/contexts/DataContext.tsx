...
export const DataProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    try {
      ...
      const poWithParts = poData.map((po) => {
        ...
        return {
          ...
          parts: poParts,
          completionDate:
            po.completion_date || (po.status === 'Completed' ? po.deadline : undefined),
        };
      });
      ...
      const mappedShipments = shipmentsData.map((shipment) => ({
        ...
        po_ids: Array.isArray(shipment['po_ids'])
          ? shipment['po_ids']
          : shipment['po_ids']
            ? [shipment['po_ids']]
            : [],
        part_ids: Array.isArray(shipment['part_ids'])
          ? shipment['part_ids']
          : shipment['part_ids']
            ? [shipment['part_ids']]
            : [],
  ...
  const updateProject = async (id: string, projectUpdate: Partial<Project>) => {
    try {
      ...
      if (projectUpdate.projectManager !== undefined)
        updateData.project_manager = projectUpdate.projectManager;
      if (projectUpdate.description !== undefined)
        updateData.description = projectUpdate.description;
  ...
  const updateSupplier = async (id: string, supplierUpdate: Partial<Supplier>) => {
    try {
      ...
      if (supplierUpdate.contactPerson !== undefined)
        updateData.contact_person = supplierUpdate.contactPerson;
      ...
      if (supplierUpdate.onTimeDelivery !== undefined)
        updateData.on_time_delivery = supplierUpdate.onTimeDelivery;
      if (supplierUpdate.positiveComments !== undefined)
        updateData.positive_comments = supplierUpdate.positiveComments;
      if (supplierUpdate.negativeComments !== undefined)
        updateData.negative_comments = supplierUpdate.negativeComments;
  ...
  const updateClient = async (id: string, clientUpdate: Partial<Client>) => {
    try {
      ...
      if (clientUpdate.contactPerson !== undefined)
      ...
      if (poUpdate.parts) {
        ...
        for (const part of poUpdate.parts) {
          if (part.id.startsWith('part-')) {
            // New part to create
            const { error: createPartError } = await supabase.from('parts').insert([
              {
                name: part.name,
                quantity: part.quantity,
                status: part.status || 'Not Started',
        ...
        const partsToDelete = Array.from(existingPartsMap.keys()).filter(
          (partId) => !updatedPartsMap.has(partId),
        );
  ...
  const deletePurchaseOrder = async (id: string) => {
    try {
      ...
      const { error: partsError } = await supabase.from('parts').delete().eq('po_id', id);
  ...
  const updateExternalLink = async (id: string, linkUpdate: Partial<ExternalLink>) => {
    try {
      ...
      setExternalLinks((prev) =>
        prev.map((link) => (link.id === id ? { ...link, ...linkUpdate } : link)),
      );
    } catch (error) {
