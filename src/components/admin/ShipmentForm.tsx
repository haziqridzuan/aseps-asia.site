...
const saveShipmentDirectly = async (shipmentData, isUpdate = false, shipmentId = null) => {
  ...
  const headers = {
    ...
    apikey:
      'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InF2bWpnY29lcGFiZmVkd2x2YmNiIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDcxMjczNzcsImV4cCI6MjA2MjcwMzM3N30.j9hFj36VwYck6jONHQRdP_Dg9-LMbcHWghbq2I3Q4ug',
    Authorization:
      'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InF2bWpnY29lcGFiZmVkd2x2YmNiIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDcxMjczNzcsImV4cCI6MjA2MjcwMzM3N30.j9hFj36VwYck6jONHQRdP_Dg9-LMbcHWghbq2I3Q4ug',
...
export default function ShipmentForm({ open, onClose, shipmentId, poId }: ShipmentFormProps) {
  ...
  useEffect(() => {
    if (formData.po_ids && formData.po_ids.length > 0) {
      ...
      setSelectedPartIds((prev) => prev.filter((id) => allParts.some((part) => part.id === id)));
    ...
    <ErrorBoundary>
      <Dialog open={open} onOpenChange={onClose}>
        <DialogContent
          ...
          aria-describedby="shipment-form-description"
        >
          <DialogHeader>
            <DialogTitle>{shipmentId ? 'Edit Shipment' : 'Create Shipment'}</DialogTitle>
          </DialogHeader>

          <p id="shipment-form-description" className="sr-only">
            Fill out the shipment details in the form below.
          </p>
              ...
              <div className="space-y-2">
                ...
                <div className="border rounded p-2 max-h-40 overflow-y-auto">
                  {purchaseOrders
                    .filter(
                      (po) =>
                        ...
                        po.projectId === formData.projectId,
                    )
                    .map((po) => (
                  ...
                  {purchaseOrders.filter(
                    (po) =>
                      po.supplierId === formData.supplierId && po.projectId === formData.projectId,
                  ).length === 0 && (
                    <span className="text-muted-foreground">
                      No POs found for this project and supplier.
                    </span>
                  )}
                </div>
              </div>
